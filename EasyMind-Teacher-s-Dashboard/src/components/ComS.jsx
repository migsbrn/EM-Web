...
const ComS = ({ onBack }) => {
  ...
  const communicationSocialAssessments = [
    ...
    {
      id: "dailyTasks",
      name: "DAILY TASKS ASSESSMENT",
      description: "Assess practical life skills and daily task completion.",
      imageUrl: "daily-tasks-illustration.png",
      bgColor: "#F3E5F5",
      textColor: "#7B1FA2",
    },
    {
    ...
    {
      id: "rhyme",
      name: "RHYME ASSESSMENT",
      description: "Assess rhyming and phonics skills.",
    ...
    {
      id: "sounds",
      name: "SOUNDS ASSESSMENT",
      description: "Assess sound recognition and pronunciation.",
      imageUrl: "sounds-illustration.png",
      bgColor: "#F1F8E9",
      textColor: "#33691E",
    },
  ]
    ...
    const fetchStudents = async () => {
      ...
      try {
        ...
        if (nicknames.length === 0) {
          // If no students are added, we set it to a value that won't match anything
          // A length-zero array will correctly be used for filtering in the main effect
          setAuthorizedNicknames([]) 
        } else {
  ...
  useEffect(() => {
    if (!selectedAssessmentType || authorizedNicknames === null) {
      setFetchedAttempts([])
      setAttemptsError(authorizedNicknames === null ? null : attemptsError) // Keep original error if any
      return
    }
  ...
  if (viewMode === "details" && selectedStudent && studentProgressData[selectedStudent]) {
    ...
    // Calculate average score to match Reports.jsx logic
    const avgScore = studentAttempts.length > 0
      ? Math.round(studentAttempts.reduce((sum, a) => sum + a.percentage, 0) / studentAttempts.length)
      : 0
    
    // Status based on average score (matching Reports.jsx)
    const isImproved = avgScore >= 70
    const status = isImproved ? 'Improved' : 'At Risk'
    ...
    console.log('Debug Progress Stats:', {
      ...
      avgScore,
      status,
      allScores: studentAttempts.map(a => a.percentage)
    })
        ...
        <div className="progress-summary-grid">
          ...
          <div className="progress-stat-card">
            ...
            <div className="progress-stat-content">
              ...
              <div className="progress-stat-value">{avgScore}%</div>
            </div>
          </div>
          <div className="progress-stat-card">
            <div className="progress-stat-icon">✅</div>
            <div className="progress-stat-content">
              ...
              <div className={`progress-stat-value ${isImproved ? 'success' : 'warning'}`} style={{ 
                fontSize: '0.9rem',
                lineHeight: '1.2',
  ...
  return (
    <div className="assessment-container">
      <div className="assessment-header">
        ...
        <div className="assessment-main-title">Communication Skills Assessment</div>
        <div className="assessment-subtitle">Track student progress in verbal, non-verbal, and comprehension abilities</div>
      </div>
        ...
        <>
          {loadingAttempts ? (
            ...
            <div className="assessment-empty-state">
              ...
              <p>No assessment results found for "{selectedAssessmentType}". Students need to complete this assessment to see data here.</p>
            </div>
          )}
        </>
      ) : (
                  ...
                  <div className="assessment-card-header">
                    <div className="assessment-card-icon-wrapper">
                      <i className={`assessment-card-icon fas ${
                        assessment.id === "pictureStory" ? "fa-image" :
                        assessment.id === "dailyTasks" ? "fa-tasks" :
                        assessment.id === "socialInteraction" ? "fa-handshake" :
                        assessment.id === "rhyme" ? "fa-book-reader" :
                        assessment.id === "sounds" ? "fa-volume-up" :
                        "fa-clipboard-check"
                      }`}></i>
                    </div>
